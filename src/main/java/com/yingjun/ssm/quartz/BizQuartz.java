 ...
 * 留空, 1970-2099 , - * /
 * 
 * 字符代表所有可能的值 / 字符用来指定数值的增量 L 字符仅被用于天（月）和天（星期）两个子表达式，表示一个月的最后一天或者一个星期的最后一天 6L
 * 可以表示倒数第６天
 * 
 * @author yingjun
 *
...
public class BizQuartz {
	...
	/**
	 * 用户自动加积分 每天9点到17点每过1分钟所有用户加一次积分 （这里要注意集群环境 可能出现重复触发的情况 ）
	 */
	...
	/**
	 * 每隔5分钟定时清理缓存 （这里要注意集群环境 可能出现重复触发的情况 ）
	 */
